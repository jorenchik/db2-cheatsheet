...
where $b_r$ -- blocks in relation $r$, same for $s$. Only *2 seeks* are required.
...
Block transfer cost: $⌈frac(b_r,(M −2))⌉ dot b_s + b_r$, $b_r$ -- blocks in relation
$r$, $M$ -- available blocks in memory buffer.
Total of $2 dot ⌈frac(b_r,(M −2))⌉$ seeks.
